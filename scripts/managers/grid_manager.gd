...
func _connect_grid_neighbors() -> void:
	...
	for x in range(grid_size.x):
		for y in range(grid_size.y):
			...
			var id = tile_data[pos]["id"]
			var z = tile_data[pos]["z_height"]
			...
			for n in neighbors:
				if tile_data.has(n) and not tile_data[n]["solid"]:
					var n_id = tile_data[n]["id"]
					var n_z = tile_data[n]["z_height"]
...
func get_grid_position(world_pos: Vector2, elevation: int = 0) -> Vector3i:
	var grid_pos = get_grid_position2d(world_pos)
	var z = elevation
	if tile_data.has(grid_pos):
...
func get_grid_path(start: Vector2i, end: Vector2i) -> Array[Vector2i]:
	...
	var start_id = tile_data[start]["id"]
	var end_id = tile_data[end]["id"]

	var point_path = astar.get_id_path(start_id, end_id)
...
func check_line_of_sight(pos_a: Vector3i, pos_b: Vector3i) -> bool:
	...
	while true:
		...
		if not (x0 == pos_a.x and y0 == pos_a.y):
			var check_pos = Vector2i(x0, y0)
			if tile_data.has(check_pos):
				var data = tile_data[check_pos]

				if data["static_los_block"]:
					return false

				var max_sight_height = max(pos_a.z, pos_b.z)
